...
* Tex Support
*/
inline.gfm.tex = /^(\${2})([^$]|[^$][\s\S]*?[^$])\1(?!\$)/;
inline.gfm.text = edit(inline.gfm.text).replace('`+', '`+|\\$+').replace('`*~', '`$*~').getRegex();
...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    if (cap = this.rules.tex.exec(src)) {
      src = src.substring(cap[0].length);
      out += this.renderer.tex(cap[2].trim());
      continue;
    }
...
Renderer.prototype.tex = function(text) {
